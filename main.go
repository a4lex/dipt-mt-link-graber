...
const (
	sqlGetMtPassword = `SELECT GROUP_CONCAT(password SEPARATOR ';') AS passwords FROM ` +
		`(SELECT password, COUNT(*) AS size FROM mt_boards GROUP BY password ORDER BY size DESC) AS t;`
	sqlGetMtList = `SELECT b.id AS board_id, INET_NTOA(b.last_ip) AS ip, b.username, b.password, ` +
		`i.id AS iface_id, i.name AS if_name, i.radio_name, i.mode FROM mt_boards b, mt_ifaces i WHERE b.id = i.mt_board_id` // AND b.id IN (1159, 1171)`
...
var (
	...
	logVerbose = flag.Int("log-level", 55, "Log verbose [ 1 - Fatal, 2 - ERROR, 4 - INFO, 8 - MYSQL, 16 - FUNC, 32 - DEBUG ]")

	threadCount = flag.Int("thread", 50, "Thread count for MT processing")
